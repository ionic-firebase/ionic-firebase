...
export class StockAddComponent {
  ...
  async addStock(stock: Stock): Promise<void> {
    try {
      ...
      const transactionDescription = `ADD ${stock.quantity} ${stock.ticker} @ ${stock.price}p`;
      ...
      this.portfolio.initialstocksbalance = Math.round((this.portfolio.initialstocksbalance + this.transactionInitialStocksCost) * 1e2) / 1e2;
      ...
      this.stockService.createStock(stock).then(res => {
        this.portfolioService.updatePortfolio(this.portfolioId, this.portfolio).then(res3 => {
          this.toastService.displayToast('Stock asset setup');
